    ...
    public function index($tesuser_id=null){
        ...
        if(!empty($tesuser_id)){
        	...
        	if($query_testuser->num_rows()>0){
				...
				$nilai = $this->cbt_tes_soal_jawaban_model->get_nilai($tesuser_id)->row();
        		$data['nilai'] =  $nilai->hasil.' (Score)';
			

        		$nilai = $this->cbt_tes_soal_model->get_nilai($tesuser_id)->row();
        		//$data['nilai'] = $nilai->hasil.' / (nilai)';
            ...
            if($temp->soal_tipe==1){
	            ...
	            if($query_jawaban->num_rows()>0){
	            	...
	            	foreach ($query_jawaban as $jawaban) {
						...
						$temp_benar = '';
						if($jawaban->poin_jawaban==1){
							$temp_benar = '<b>1</b>';
						} else if ($jawaban->poin_jawaban==2){
								$temp_benar = '<b>2</b>';
						} else if ($jawaban->poin_jawaban==3){
								$temp_benar = '<b>3</b>';
						} else if ($jawaban->poin_jawaban==4){
								$temp_benar = '<b>4</b>';
						} else if ($jawaban->poin_jawaban==5){
								$temp_benar = '<b>5</b>';
						} else {
							$temp_benar = '<b>0</b>';
						}
