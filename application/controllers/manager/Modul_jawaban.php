...
class Modul_jawaban extends Member_Controller {
        ...
        if($id_soal!=null){
	        ...
	        if($id_jawaban!=null){
	        	...
	        	if($query_jawaban->num_rows()>0){
					...
					$data['tambah-poin'] = $query_jawaban->poin_jawaban;
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('tambah-poin', 'Poin Jawaban','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
			...
			$data['poin_jawaban'] = $this->input->post('tambah-poin', TRUE);
    ...
    function get_by_id($id=null){
		...
		if(!empty($id)){
			...
			if($query->num_rows()>0){
				...
				$data['tambah-poin'] = $query->poin_jawaban;
